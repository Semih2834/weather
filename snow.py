...
# Create the snowy_days table
session.execute("""
CREATE TABLE IF NOT EXISTS snowy_days (
    station text,
    date date,
    humidity float,
    prcp float,
...
""")
print("Table snowy_days created.")

# Query to fetch entries with snow > 0
select_query = "SELECT * FROM new_jersey_weather WHERE snow > 0 ALLOW FILTERING"

# Prepare insert query for the new table
insert_query = """
INSERT INTO snowy_days (station, date, humidity, prcp, snow, tmax, tmin, wind)
...
for row in rows:
    try:
        # Check if snow is NaN and skip such rows
        if row.snow is None or isnan(row.snow):
            print(f"Skipping row due to NaN snow value: {row}")
            continue
...
print("Filtered data inserted into snowy_days table.")
